...
typedef uint64_t local_bindings_types_duration_t;

// Configuration for constant arrival rate
typedef struct local_bindings_types_constant_arrival_rate_config_t {
  local_bindings_types_duration_t   duration;
  uint64_t   rate;
  local_bindings_types_duration_t   time_unit;
  uint64_t   allocated_vus;
  local_bindings_types_duration_t   graceful_shutdown_timeout;
} local_bindings_types_constant_arrival_rate_config_t;

// Configuration variants
typedef struct local_bindings_types_config_t {
  uint8_t tag;
  union {
    local_bindings_types_constant_arrival_rate_config_t     constant_arrival_rate;
  } val;
} local_bindings_types_config_t;

#define LOCAL_BINDINGS_TYPES_CONFIG_CONSTANT_ARRIVAL_RATE 0

typedef local_bindings_types_config_t bindings_config_t;
...
// Exported Functions from `bindings`
void bindings_get_config(bindings_config_t *ret);
void bindings_run_scenario(void);
...
void local_bindings_types_config_free(local_bindings_types_config_t *ptr);

void bindings_config_free(bindings_config_t *ptr);
